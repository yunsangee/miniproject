...
function fncAddPurchase() {
	...
	var buyer = $("input[name='receiverName']").val();
	var phoneNum = $("input[name='reveiverPhone']").val();
	var address = $("input[name='divyAddr']").val();
	var deliverDate = $("input[name='divyDate']").val();
	
	if(paymentOption == null || paymentOption <1){
		alter("구매 방식은 반드시 입력하셔야 합니다.");
		return;
	}
	if(buyer == null || buyer <1){
		alter("구매자 이름은 반드시 입력하셔야 합니다.");
		return;
	}
	if(phoneNum == null || phoneNum <1){
		alter("구매자 전화번호는 반드시 입력하셔야 합니다.");
		return;
	}
	...
	if(deliverDate == null || deliverDate <1){
		alter("구매 방식은 반드시 입력하셔야 합니다.");
		return;
	}
